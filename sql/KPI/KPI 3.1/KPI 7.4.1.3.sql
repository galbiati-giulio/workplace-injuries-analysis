...
SELECT
    ROUND(MAX(CASE WHEN sex_id = 'M' THEN avg_employment END), 2) AS avg_male_employment,
    ROUND(MAX(CASE WHEN sex_id = 'F' THEN avg_employment END), 2) AS avg_female_employment,
    ROUND(
        MAX(CASE WHEN sex_id = 'M' THEN avg_employment END) * 1.0
        / MAX(CASE WHEN sex_id = 'F' THEN avg_employment END),
        2
    ) AS gender_employment_ratio
