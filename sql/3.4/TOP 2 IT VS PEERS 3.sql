...
top10_avg AS (
    SELECT
        category_id,
        subcategory_id,
        ROUND(AVG(share_in_category) * 100, 2) AS subcategory_share_within_category_pct
    FROM top10_shares
...
ranked AS (
    SELECT
        category_id,
        subcategory_id,
        subcategory_share_within_category_pct,
        ROW_NUMBER() OVER (
            PARTITION BY category_id
            ORDER BY subcategory_share_within_category_pct DESC, subcategory_id
        ) AS rn
    FROM top10_avg
),
...
unioned AS (
    SELECT
        ...
        'Male' AS gender,
        a.male_share_pct AS gender_share_pct,
        s.subcategory_share_within_category_pct,
        1 AS gender_order
    FROM avg_gender a
    JOIN selected s
    ...
    SELECT
        ...
        sc.subcategory_description,
        'Female' AS gender,
        a.female_share_pct AS gender_share_pct,
        s.subcategory_share_within_category_pct,
        2 AS gender_order
    FROM avg_gender a
    JOIN selected s
...
ORDER BY
    category_description,
    subcategory_share_within_category_pct DESC,
    subcategory_description,
    country_scope,
    gender_order;